...
mysql_select_db("a5091678_ecommer");
 ...
 echo "<hr>";
              ...
              echo "<center><h2>Change tune mama</h2></center>";
 echo "<a href='balls.php'><h2>Banarasi Balls</h2></a>";

 $sql="SELECT * FROM sports_data WHERE `sub_id`=1  ORDER BY `time` DESC LIMIT 5";
 ...
 echo "<hr>";
            ...
            //  echo "<center><h2>Change tune mama</h2></center>";
 echo "<a href='bats.php'><h2>Banarasi Bats</h2></a>";

 $sql="SELECT * FROM sports_data WHERE `sub_id`=2  ORDER BY `time` DESC LIMIT 5";
 $res= mysql_query($sql);$a;
 echo '<table><tr>' ;
 while($row=mysql_fetch_assoc($res)){
       $result=    mysql_query("SELECT `name` FROM `sports_2` WHERE `sub_id`=".$row['sub_id']."");
