...
mysql_select_db("a5091678_ecommer");
 ...
 echo "<hr>";
              ...
              echo "<center><h2>Sutruv kapad udhyog</h2></center>";
 echo "<a href='shirts.php'><h2>Shirt</h2></a>";

 $sql="SELECT * FROM cloths_data WHERE `sub_id`=1  ORDER BY `time` DESC LIMIT 5";
 ...
 echo "<a href='t-shirts.php'><h2>T-shirts</h2></a>";

 echo "<hr>";


 $sql="SELECT * FROM cloths_data WHERE `sub_id`=2  ORDER BY `time` DESC LIMIT 5";
 $res= mysql_query($sql);$a;
 echo '<table><tr>' ;
 while($row=mysql_fetch_assoc($res)){
       $result=    mysql_query("SELECT `name` FROM `cloths_2` WHERE `sub_id`=".$row['sub_id']."");
 ...
 echo "<hr>" ;
 echo "<a href='jackets.php'><h2>Jackets</h2></a>";

 echo "<hr>";



 $sql="SELECT * FROM cloths_data WHERE `sub_id`=3  ORDER BY `time` DESC LIMIT 5";
 $res= mysql_query($sql);$a;
 echo '<table><tr>' ;
 while($row=mysql_fetch_assoc($res)){
       $result=    mysql_query("SELECT `name` FROM `cloths_2` WHERE `sub_id`=".$row['sub_id']."");
